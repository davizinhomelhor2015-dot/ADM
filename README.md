...
2. Os registros enviados pela Ouvidoria devem aparecer na lista. Como a Ouvidoria agora **bloqueia o envio** de mensagens com linguagem imprópria (nada com palavrão chega a ser salvo), o selo ⚠ Linguagem imprópria não deve mais aparecer em registros novos — isso é esperado.
...
- **Linguagem imprópria agora bloqueia o envio** (na Ouvidoria), em vez de só marcar. Como consequência, registros novos não devem mais chegar com o selo ⚠ Linguagem imprópria — essas mensagens nunca chegam a ser salvas.
...
```
/suggestions/{id}: name, message, ip, deviceId, fingerprint, userAgent, status, createdAt
...
> Registros salvos antes desta atualização podem ainda ter os campos `hasProfanity`/`profanityWords` (de quando a Ouvidoria só marcava em vez de bloquear) — o painel continua lendo esses campos normalmente se existirem, só não são mais gerados para registros novos.
